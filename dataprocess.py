...
def process_directory(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            print(file)
            if file.endswith(".bpseq") and not file.startswith(".") and not file.endswith("-checkpoint.bpseq"):
...
if __name__ == '__main__':
    # 定义要处理的目录
    directory = "/mnt/workspace//xx/test/"
    RNA_SS_data = collections.namedtuple('RNA_SS_data','seq name length one_hot_matrix feature label_matrix edge_index edge_type')
    random.seed(4)
    all_files_list = process_directory(directory)
    print(len(all_files_list))
    cPickle.dump(all_files_list, open("/mnt/workspace//22.cPickle", "wb"))
    # RNA_SS_train,RNA_SS_test = train_test_split(all_files_list,
    #                                         test_size=0.2,random_state=0)
    # print(len(RNA_SS_train))
    # print(len(RNA_SS_test))
    # cPickle.dump(RNA_SS_train, open("/mnt/workspace/16S_rRNA_database_train.cPickle", "wb"))
    # cPickle.dump(RNA_SS_test, open("/mnt/workspace/16S_rRNA_database_test.cPickle", "wb"))